...
void Tree::getTreeInfo(cJSON* ptmObj, cJSON* multiresObj, const QString& multiresStrategyId)
{
    cJSON *maxResObj;
    cJSON_AddItemToObject(ptmObj, "maxResolution", maxResObj = cJSON_CreateObject());
    cJSON_AddNumberToObject(maxResObj, "w", maxSize);
    cJSON_AddNumberToObject(maxResObj, "h", maxSize);

    cJSON *tileSizeObj;
    cJSON_AddItemToObject(multiresObj, "tileSize", tileSizeObj = cJSON_CreateObject());
    if (multiresStrategyId == "IIIF") {
        cJSON_AddNumberToObject(tileSizeObj, "w", 255);
        cJSON_AddNumberToObject(tileSizeObj, "h", 255);
    } else {
        cJSON_AddNumberToObject(tileSizeObj, "w", tileSize);
        cJSON_AddNumberToObject(tileSizeObj, "h", tileSize);
    }
}
...
void Splitter::saveDescriptorXML(const QString& destFolder, const QString& format)
{
    ...
    QDomElement rotElem = doc.createElement("Orientation");
    QString strOrinent = QString::number(image->orientation());
    rotElem.appendChild(doc.createTextNode(strOrinent));

	root.appendChild(content);
	root.appendChild(treeElem);
    root.appendChild(rotElem);
...
void Splitter::saveDescriptorJSON(const QString& destFolder, const QString& format, const QString& geometryId,  const QString& multiresStrategyId)
{
    QString fileDescr = destFolder + "/info.json";

    cJSON *root,*ptm, *geom, *multiresStrategy;
    root = cJSON_CreateObject();

    cJSON_AddItemToObject(root, "PTM", ptm = cJSON_CreateObject());
    cJSON_AddItemToObject(root, "Geometry", geom = cJSON_CreateObject());
    cJSON_AddItemToObject(root, "MultiresStrategy", multiresStrategy = cJSON_CreateObject());

    image->getLayerInfo(ptm);
    cJSON_AddStringToObject(ptm, "imageFormat", format.toUtf8().constData());
    cJSON_AddStringToObject(geom, "type", geometryId.toUtf8().constData());
    cJSON_AddStringToObject(multiresStrategy, "type", multiresStrategyId.toUtf8().constData());

    tree.getTreeInfo(ptm, multiresStrategy, multiresStrategyId);

    QString qJSONStr = QString(cJSON_Print(root));
    QFile file(fileDescr);
    if (file.open(QIODevice::WriteOnly))
    {
        QTextStream stream(&file);
        stream << qJSONStr  << endl;
        file.close();
    }
