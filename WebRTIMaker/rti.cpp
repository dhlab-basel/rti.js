...
bool Rti::loadRti(const QString& filename, MultiLayerImage** image)
{
	...
	if (type.find("PTM") != std::string::npos)
	{
		fscanf(file, "%s", s);
		std::string format(s);
        if (format.find("PTM_FORMAT_LRGB_GLOSS") != std::string::npos)
            *image = new PtmLRGBGloss();
		else if (format.find("PTM_FORMAT_RGB") != std::string::npos)
			*image = new PtmRGB();
        else if (format.find("PTM_FORMAT_LRGB") != std::string::npos)
            *image = new PtmLRGB();
...
void Hsh::getLayerInfo(cJSON* jsonObj)
{
    cJSON_AddStringToObject(jsonObj, "notice", "JSON info not yet implemented for HSH");
}
...
void PtmLRGB::getLayerInfo(cJSON* jsonObj)
{
    cJSON_AddStringToObject(jsonObj, "type", "LRGB_PTM");

    cJSON *contentSize;
    cJSON_AddItemToObject(jsonObj, "contentSize", contentSize = cJSON_CreateObject());
    cJSON_AddNumberToObject(contentSize, "w", this->width());
    cJSON_AddNumberToObject(contentSize, "h", this->height());

    cJSON_AddItemToObject(jsonObj, "scale", cJSON_CreateFloatArray(scale, 6));
    cJSON_AddItemToObject(jsonObj, "bias", cJSON_CreateFloatArray(bias, 6));
    cJSON_AddNumberToObject(jsonObj, "orientation", rotAngle);
}
...
//LRGBGloss Polynomial Texture Map
PtmLRGBGloss::PtmLRGBGloss() : Rti()
{
    coeffNumber = 4;
    for (int i = 0; i < 4; i++)
        this->coeff[i] = NULL;
...
PtmLRGBGloss::~PtmLRGBGloss()
{
    releaseMemory();
}


bool PtmLRGBGloss::loadData()
{
     if (isNull())
        return false;
    releaseMemory();
    ...
    if (true){
        ...
        if (clippingRect.y()> 0)
        {
            long offset = w * 7 * (clippingRect.y());
            fseek(file, offset, SEEK_CUR);
        }
        xOffset = clippingRect.x() * 3;
        xOffsetAfter = (w - clippingRect.right() - 1) * 3;
        ...
        for (int y = clippingRect.y(); y < clippingRect.y() + clippingRect.height(); y++)
        {
            if (xOffset > 0)
                fseek(file, xOffset, SEEK_CUR);
            if (fread(&coeff[3][offset], sizeof(unsigned char), line_size, file) < line_size)
            {
                error = true;
                break;
            }
...
void PtmLRGBGloss::releaseMemory()
{
    for (int i = 0; i < 4; i++)
    {
        if (coeff[i])
...
bool PtmLRGBGloss::getLayer(int layerIndex, QImage* layer)
{
    if (!loaded || layerIndex < 0 || layerIndex > coeffNumber)
        return false;
    if (layer == NULL)
...
void PtmLRGBGloss::getLayerInfo(QDomDocument& doc, QDomElement& content)
{
    content.setAttribute("type", type);
    ...
    sizeElem.setAttribute("width", this->width());
    sizeElem.setAttribute("height", this->height());
    sizeElem.setAttribute("coefficients", 4);
...
void PtmLRGBGloss::getLayerInfo(cJSON* jsonObj)
{
    cJSON_AddStringToObject(jsonObj, "type", "LRGBG_PTM");
    ...
    cJSON_AddItemToObject(jsonObj, "scale", cJSON_CreateFloatArray(scale, 9));
    cJSON_AddItemToObject(jsonObj, "bias", cJSON_CreateFloatArray(bias, 9));
    cJSON_AddNumberToObject(jsonObj, "orientation", rotAngle);
}


bool PtmLRGBGloss::loadHeader()
{
    #ifdef WIN32
#ifndef __MINGW32__
    FILE* file;
    ...
    //Get Rotation Angle
    QString str1 = getLine(file, &eof);
    if (eof) return false;
    rotAngle = str1.toUInt(&error);
    if (!error) return false;
...
void PtmRGB::getLayerInfo(cJSON* jsonObj)
{
    cJSON_AddStringToObject(jsonObj, "notice", "JSON info not yet implemented for PTMRGB");
}
